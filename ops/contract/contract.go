...
import (
	...
	operation "github.com/yumauri/fbrcm/ops/invocation"
...
func Enabled(cmd operation.Call) bool {
	if cmd == nil {
		return false
	}
...
func CommandID(cmd operation.Call) string {
	if cmd == nil {
		return "root"
	}
...
func BuildEnvelope(cmd operation.Call, version string, captured []byte, err error) Envelope {
...
func envelopeCommandID(cmd operation.Call) string {
	if cmd != nil {
...
func requestedCommandID(cmd operation.Call) string {
	if cmd == nil {
		return "root"
	}
...
func boolFlag(cmd operation.Call, name string) bool {
	if cmd == nil {
		return false
	}
...
func ExitCode(cmd operation.Call, err error) int {
	if err == nil {
		return 0
	}
...
func classifyCommandError(cmd operation.Call, command string, err error) (Problem, bool) {
...
func collectedWarnings(cmd operation.Call) []Warning {
