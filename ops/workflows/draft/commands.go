...
func NewDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{Use: "draft", Short: "Inspect, publish, and discard Remote Config drafts"}
	cmd.AddCommand(newListCommandDefinition(), newPathCommandDefinition(), newShowCommandDefinition(), newChangeNoteCommandDefinition(svc), newDiffCommandDefinition(svc), newPublishCommandDefinition(svc), newDiscardCommandDefinition())
	invocation.MustRegisterResponsePath(cmd, "list", []listItem{})
	invocation.MustRegisterResponsePath(cmd, "path", shared.PathResult{})
	invocation.MustRegisterResponsePath(cmd, "show", contract.ArtifactData{})
	invocation.MustRegisterResponsePath(cmd, "change-note", draftChangeNoteResult{})
	invocation.MustRegisterResponsePath(cmd, "diff", draftDiffResult{})
	invocation.MustRegisterResponsePath(cmd, "publish", []publishResult{}, rc.PlanCreatedResult{})
	invocation.MustRegisterResponsePath(cmd, "discard", []draftDiscardResult{})
	return cmd
}

func newPathCommandDefinition() *invocation.Definition {
	cmd := &invocation.Definition{
		Use:   "path",
		Short: "Print Remote Config drafts directory path",
		Args:  invocation.NoArgs,
		RunE: func(cmd invocation.Call, _ []string) error {
...
func newListCommandDefinition() *invocation.Definition {
	cmd := &invocation.Definition{Use: "list", Short: "List local Remote Config drafts", Args: invocation.NoArgs, RunE: runList}
...
func runList(cmd invocation.Call, _ []string) error {
...
func newShowCommandDefinition() *invocation.Definition {
	cmd := &invocation.Definition{Use: "show <project>", Short: "Show a local Remote Config draft", Args: invocation.ExactArgs(1), RunE: runShow}
...
func newChangeNoteCommandDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{
...
func newDiffCommandDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{Use: "diff <project>", Short: "Compare a draft with its base or current Remote Config", Args: invocation.ExactArgs(1), RunE: func(cmd invocation.Call, args []string) error {
...
func runDiff(cmd invocation.Call, svc *core.Core, query string) error {
...
func newPublishCommandDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{Use: "publish [project...]", Short: "Safely rebase and publish Remote Config drafts", Args: invocation.ArbitraryArgs, RunE: func(cmd invocation.Call, args []string) error {
...
func runPublish(cmd invocation.Call, svc *core.Core, args []string) error {
...
func writeDraftPublicationPlan(ctx context.Context, cmd invocation.Call, svc *core.Core, ids []string, overrideNote *string, path string) error {
	environment, err := core.PublicationEnvironmentForContext(ctx)
	if err != nil {
		return err
	}
	publicationPlan := publication.New(invocation.Version(cmd), "draft.publish", environment.Policy, overrideNote)
...
func newDiscardCommandDefinition() *invocation.Definition {
	cmd := &invocation.Definition{Use: "discard [project...]", Short: "Discard local Remote Config drafts", Args: invocation.ArbitraryArgs, RunE: runDiscard}
...
func runDiscard(cmd invocation.Call, args []string) error {
	ids, err := selectedDraftIDs(cmd, args)
	if err != nil {
		return err
	}
