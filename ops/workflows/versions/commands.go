...
func NewDefinition(svc *core.Core) *invocation.Definition {
	...
	invocation.MustRegisterResponsePath(cmd, "export", contract.ArtifactData{})
	invocation.MustRegisterResponsePath(cmd, "rollback", versionPublishResult{})
	invocation.MustRegisterResponsePath(cmd, "restore", versionPublishResult{}, rc.PlanCreatedResult{})
	return cmd
}

func resolveVersionProject(cmd invocation.Call, svc *core.Core, query string, cachedOnly bool) (core.Project, error) {
...
func newVersionsListCommandDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{Use: "list <project>", Short: "List Remote Config versions", Args: invocation.ExactArgs(1), RunE: func(cmd invocation.Call, args []string) error {
...
func timeFlag(cmd invocation.Call, name string) (time.Time, error) {
...
func newVersionsShowCommandDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{Use: "show <project> <version>", Short: "Show Remote Config version metadata or JSON", Args: invocation.ExactArgs(2), RunE: func(cmd invocation.Call, args []string) error {
		cached, _ := cmd.Flags().GetBool("cached")
		project, err := resolveVersionProject(cmd, svc, args[0], cached)
		if err != nil {
			return err
...
func newVersionsDiffCommandDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{Use: "diff <project> <from> [<to>]", Short: "Compare two Remote Config versions", Args: invocation.RangeArgs(2, 3), RunE: func(cmd invocation.Call, args []string) error {
...
func addVersionDiffFlags(cmd invocation.FlagGroups) {
...
func runVersionsDiff(cmd invocation.Call, svc *core.Core, args []string) error {
...
func readVersionDiffOptions(cmd invocation.Call) versionDiffOptions {
...
func newVersionsExportCommandDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{Use: "export <project> <version>", Short: "Export historical Remote Config JSON", Args: invocation.ExactArgs(2), RunE: func(cmd invocation.Call, args []string) error {
		cached, _ := cmd.Flags().GetBool("cached")
		project, err := resolveVersionProject(cmd, svc, args[0], cached)
		if err != nil {
			return err
...
func newVersionsRollbackCommandDefinition(svc *core.Core, restore bool) *invocation.Definition {
	...
	cmd := &invocation.Definition{Use: name + " <project> <version>", Short: short, Args: invocation.ExactArgs(2), RunE: func(cmd invocation.Call, args []string) error {
...
func runVersionPublish(cmd invocation.Call, svc *core.Core, query, selector string, restore bool) error {
	ctx := shared.CommandContext(cmd)
	...
	if planPath, planning, planErr := shared.PlanOutputPath(cmd); planErr != nil {
		...
		publicationPlan := publication.New(invocation.Version(cmd), "versions.restore", environment.Policy, changeNote)
