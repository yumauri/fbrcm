...
func Run(cmd invocation.Call, svc *core.Core, project core.Project) error {
	...
	if planPath, planning, planErr := shared.PlanOutputPath(cmd); planErr != nil {
		...
		publicationPlan := publication.New(invocation.Version(cmd), "project.import", environment.Policy, changeNote)
...
func writeImportResult(cmd invocation.Call, jsonOut bool, result importResult) error {
	if jsonOut {
...
func readImportOptions(cmd invocation.Call) (importOptions, error) {
...
func readRemoteConfig(cmd invocation.Call) ([]byte, error) {
...
func buildFinalImportConfig(cmd invocation.Call, currentCfg, importCfg *firebase.RemoteConfig, opts importOptions) (*firebase.RemoteConfig, error) {
...
func chooseImportStrategy(cmd invocation.Call, opts importOptions) (importStrategy, error) {
	switch {
	case opts.override:
