...
func NewDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{Use: "groups", Short: "Inspect and manage Remote Config parameter groups"}
	cmd.AddCommand(newListCommandDefinition(svc), newAddCommandDefinition(svc), newEditCommandDefinition(svc), newRenameCommandDefinition(svc), newDeleteCommandDefinition(svc))
	invocation.MustRegisterResponsePath(cmd, "list", []groupListJSON{})
...
func newListCommandDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{
		Use: "list", Short: "List parameter groups across projects", Args: invocation.NoArgs,
...
func loadProjects(cmd invocation.Call, svc *core.Core) ([]loadedGroups, error) {
