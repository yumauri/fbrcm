...
func NewDefinition(svc *core.Core) *invocation.Definition {
	cmd := &invocation.Definition{Use: "apply <plan>", Short: "Apply an immutable publication plan", Args: invocation.ExactArgs(1), RunE: func(cmd invocation.Call, args []string) error {
	...
	invocation.RegisterResponse(cmd, result{})
	return cmd
}

func run(cmd invocation.Call, svc *core.Core, path string) error {
...
func addNonAtomicWarning(cmd invocation.Call, publishTargets int, dryRun bool) {
...
func classifyPublishResult(cmd invocation.Call, target string, entry targetResult, publishedRaw json.RawMessage, publishErr error) (targetResult, bool) {
...
func cleanupAlreadyAppliedDrafts(cmd invocation.Call, svc *core.Core, plan *publication.Plan, items []targetResult) {
...
func cleanupMatchingDraft(cmd invocation.Call, svc *core.Core, target publication.Target) {
...
func writeResult(cmd invocation.Call, value result) error {
	if contract.Enabled(cmd) {
