...
func TestParseProjectRemoteConfigDecodeErrorIncludesProjectID(t *testing.T) {
	_, err := ParseProjectRemoteConfig("demo", []byte(`{`))
	if err == nil {
		t.Fatalf("ParseProjectRemoteConfig accepted invalid config")
	}
	if !strings.Contains(err.Error(), "demo") {
		t.Fatalf("error = %q, want project id", err.Error())
	}
...
func TestParseCachedProjectRemoteConfigDecodeErrorIncludesProjectID(t *testing.T) {
	_, err := ParseCachedProjectRemoteConfig("demo", []byte(`{`))
	if err == nil {
		t.Fatalf("ParseCachedProjectRemoteConfig accepted invalid config")
	}
	if !strings.Contains(err.Error(), "cached") || !strings.Contains(err.Error(), "demo") {
		t.Fatalf("error = %q, want cached project id", err.Error())
	}
}
