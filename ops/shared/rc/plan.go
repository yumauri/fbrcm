...
func runRemotePlanLoop(ctx context.Context, cmd invocation.Call, svc *core.Core, projects []core.Project, defaultScope bool, operation, path string, planner RemoteMutationPlanner) (RemoteMutationTotals, error) {
	...
	plan := publication.New(invocation.Version(cmd), operation, environment.Policy, remoteMutationChangeNote(ctx))
...
func WritePublicationPlan(cmd invocation.Call, plan *publication.Plan, path string) (*PlanCreatedResult, error) {
	if err := publication.Seal(plan); err != nil {
		return nil, err
	}
	raw, err := publication.Marshal(plan)
