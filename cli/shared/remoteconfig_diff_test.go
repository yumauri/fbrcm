...
func TestFormatPlainValue(t *testing.T) {
	tests := []struct {
		...
		{name: "simple", in: "enabled", want: "enabled"},
...
func TestFormatRemoteValue(t *testing.T) {
	tests := []struct {
		name  string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := FormatRemoteValue(tt.value); got != tt.want {
				t.Fatalf("FormatRemoteValue() = %q, want %q", got, tt.want)
			}
		})
	}
}
