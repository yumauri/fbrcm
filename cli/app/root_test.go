...
func TestNewRootCommandBuildsFreshRoot(t *testing.T) {
	...
	if got, want := commandNames(first), []string{"add", "auth", "cache", "config", "delete", "draft", "get", "profile", "project", "projects", "update", "versions"}; !reflect.DeepEqual(got, want) {
...
func TestRootCommandConstructionDoesNotAccumulateSubcommands(t *testing.T) {
	...
	if !reflect.DeepEqual(counts, []int{12, 12, 12}) {
